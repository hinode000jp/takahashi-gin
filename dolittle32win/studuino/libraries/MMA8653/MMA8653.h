...
class MMA8653
{
  public:
    ...
    // Added and modified by kagayama@ArTec 2014/7/23
    // Original get[XYZ] (float) were converted to get[XYZ]G (float).
    // int16_t get[XYZ] were newly defined.
    float getXG();
    float getYG();
    float getZG();
    int16_t getX();
    ...
    float getTheta();
    void update();
    // Added by kagayama@ArTec 2014/7/23
    bool testConnection();
