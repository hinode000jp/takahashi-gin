...
Dim libpath
libpath = "studuino\hardware\arduino\avr\build\studuinomini"

Dim buildpath
buildpath = "studuino\build"

Dim HIDDTU_PATH
HIDDTU_PATH = selfpath + "studuino\hardware\tools\bootloadHID"
...
MsgBox ("プログラムを転送します。「OK」ボタンを押した後、２秒以内に基板のリセットボタンを押してください。")
'WScript.StdOut.WriteLine "Studuino基板へのプログラム転送を開始"
ret = SendProgram(buildpath + "\studuino.cpp")
If (ret <> 0) Then
...
Function CompileSource(filepath)
    ...
    Dim strPrm
    strPrm = " -c -g -Os -w -fno-exceptions -ffunction-sections -fdata-sections -fno-threadsafe-statics -MMD -mmcu=atmega168 -DF_CPU=12000000L -DARDUINO=10603 -DARDUINO_AVR_PRO -DARDUINO_ARCH_AVR" _
             ...
             + " -Istuduino\libraries\Clock" _
             + " -Istuduino\libraries\StuduinoMini " _
...
Function LinkObjects(filepath)
    ...
    strPrm = " -w -Os -Wl,--gc-sections -mmcu=""atmega168"" " _
             ...
             + libpath + "\StuduinoMini.a " + libpath + "\core.a " + "-lm"
    'WScript.StdOut.WriteLine strCmd + strPrm
    Set WshShell = CreateObject("WScript.Shell")
    Set outExec = WshShell.Exec(strCmd + strPrm)
    Do While outExec.Status = 0
...
Function SendProgram(filepath)
    Dim WshShell, ExitCode
    


    Dim strCmd
    strCmd = HIDDTU_PATH + "\bhid.bat"
    
    Dim strPrm
    strPrm = " "+ selfpath + filepath + ".hex"
    WScript.StdOut.WriteLine strCmd + strPrm
    Set WshShell = CreateObject("WScript.Shell")
    'Set outExec = WshShell.Exec(strCmd + strPrm)
    ExitCode = WshShell.Run(strCmd + strPrm,7,true)

    SendProgram = ExitCode
    Set WshShell = Nothing
